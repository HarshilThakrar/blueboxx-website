...
export default function BlogPage() {
  ...
  const { data: categories = [] } = useSWR("/public/blog-categories", categoryFetcher, {
  ...
  const { data: blogsData, isLoading } = useSWR(['/public/blogs', currentPage, debouncedSearchQuery, activeCategory, categories.length], ([url]) => blogFetcher(url), {
    revalidateOnFocus: false,
                  ...
                  <Card className="overflow-hidden group hover:shadow-xl hover:border-[#1B2A6B]/30 transition-all duration-300">
                    <div className="flex flex-col lg:flex-row h-full">
                      <div className="w-full lg:w-1/2 aspect-[16/9] lg:aspect-auto relative overflow-hidden bg-slate-200">
                        <Image src={featuredBlog.thumbnail ? `http://localhost:8000/storage/${featuredBlog.thumbnail}` : "https://images.unsplash.com/photo-1522071820081-009f0129c71c?w=800&q=80"} alt="Featured" fill className="object-cover group-hover:scale-105 transition-transform duration-700" sizes="(max-width: 1024px) 100vw, 50vw" />
                      </div>
                  ...
                  {remainingBlogs.map((blog: any) => (
                    <Card key={blog.id} className="overflow-hidden group hover:shadow-lg hover:border-[#1B2A6B]/30 transition-all duration-300 flex flex-col">
                      <div className="relative aspect-[16/9] overflow-hidden bg-slate-200">
                        <Image src={blog.thumbnail ? `http://localhost:8000/storage/${blog.thumbnail}` : "https://images.unsplash.com/photo-1555066931-4365d14bab8c?w=800&q=80"} alt={blog.title} fill className="object-cover group-hover:scale-105 transition-transform duration-700" sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw" />
          ...
          <div className="bg-[#0d1635] rounded-3xl p-10 md:p-16 text-center text-white relative overflow-hidden border border-white/10 shadow-2xl">
            ...
            <div className="relative z-10 max-w-2xl mx-auto">
              ...
              <form onSubmit={async (e) => {
                ...
                if(!email) return;
                try {
                  const res = await api.post('/public/newsletter/subscribe', { email });
                ...
                <Button type="submit" variant="gold" className="py-3">Subscribe</Button>
              </form>
            </div>
          </div>
