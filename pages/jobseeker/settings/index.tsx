...
import { Bell, Shield, Briefcase, Mail, User, Smartphone, CheckCircle2 } from "lucide-react";
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div className="p-6 space-y-5">
              <div className="flex items-center gap-6">
                ...
                <div>
                  <label className="inline-block cursor-pointer px-4 py-2 bg-slate-50 border border-slate-200 text-slate-700 text-xs font-bold rounded-lg hover:bg-slate-100 transition-colors shadow-sm">
                    Change Avatar
                    <input type="file" className="hidden" accept="image/*" onChange={(e) => {
                      if(e.target.files && e.target.files.length > 0) toast.success("Avatar updated!");
                    }} />
                  </label>
              ...
              <div className="flex justify-end border-t border-slate-100 pt-5 mt-2">
                <button onClick={() => toast.success("Changes saved successfully!")} className="px-5 py-2.5 bg-[#1B2A6B] text-white text-sm font-bold rounded-xl shadow-md hover:bg-[#0d1635] transition-colors">
                  Save Changes
                </button>
              </div>
            </div>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="flex items-center justify-between group">
                <div className="flex gap-4 items-start">
                  ...
                  <div>
                    <h3 className="font-bold text-slate-800 text-sm mb-0.5">Job Alerts</h3>
                    <p className="text-xs font-semibold text-slate-500 max-w-sm">Get emails when top companies post jobs matching your skills.</p>
                  </div>
                </div>
                <button 
                  onClick={() => setJobAlerts(!jobAlerts)} 
                  className={`w-11 h-6 rounded-full relative transition-colors shadow-inner shrink-0 flex items-center ${jobAlerts ? 'bg-[#1B2A6B]' : 'bg-slate-200'}`}
                >
                  <motion.div layout className="w-4 h-4 bg-white rounded-full shadow-sm mx-1" animate={{ x: jobAlerts ? 20 : 0 }} transition={{ type: "spring", stiffness: 500, damping: 30 }} />
                </button>
              </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-gradient-to-br from-[#0d1635] to-[#1B2A6B] rounded-2xl p-6 text-white shadow-md relative overflow-hidden">
             ...
             <div className="space-y-4">
               ...
               <button onClick={() => toast.success("Password reset link sent to your email")} className="w-full py-2.5 bg-[#C9A227] hover:bg-[#b08d22] text-[#0d1635] text-sm font-bold rounded-xl transition-colors shadow-sm">
                 Change Password
               </button>
             </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
             ...
             <div className="flex items-center gap-3 p-3 bg-emerald-50 rounded-xl border border-emerald-100">
               ...
               <div>
                 <p className="text-xs font-black text-emerald-800">Verified Jobseeker Account</p>
