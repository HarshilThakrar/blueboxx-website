...
import { HelpCircle, ChevronDown, ChevronUp, MessageSquare, Send, Mail, Phone, CheckCircle2 } from "lucide-react";
...
export default function SupportPage() {
  ...
  const handleSend = (e: React.FormEvent) => {
    e.preventDefault();
    if (message.trim()) { setSent(true); setTimeout(() => { setSent(false); setMessage(""); }, 3000); }
  };
            ...
            {FAQS.map((faq, i) => (
              <AnimatedContent key={i} direction="up" delay={i * 0.06} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                >
                  ...
                  {expanded === i ? <ChevronUp size={16} className="text-slate-400 shrink-0" /> : <ChevronDown size={16} className="text-slate-400 shrink-0" />}
                </button>
                {expanded === i && (
          ...
          <AnimatedContent direction="up" delay={0.3} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
            ...
            {sent ? (
              <div className="flex flex-col items-center py-6 text-center">
                ...
                <p className="font-black text-slate-800 text-sm">Message Sent!</p>
                <p className="text-xs text-slate-400 font-semibold mt-1">We'll respond within 24 hours.</p>
              </div>
            ) : (
              <form onSubmit={handleSend} className="space-y-3">
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase mb-1.5">Subject</label>
                  <select className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-[#1B2A6B] outline-none">
                ...
                <button
                  type="submit"
                  className="w-full flex items-center justify-center gap-2 py-2.5 bg-[#1B2A6B] text-white font-bold rounded-xl hover:bg-[#0d1635] transition-colors text-sm"
                >
