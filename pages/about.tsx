        ...
        <section className="bg-[#0d1635] pt-24 pb-16 px-4 text-center relative overflow-hidden">
          ...
          <div className="container mx-auto max-w-4xl relative z-10">
            ...
            >
              ...
              <span className="text-[#C9A227]">Meets Innovation.</span>
            </motion.h1>
                  ...
                  <div className="space-y-6 text-slate-600 text-lg leading-relaxed">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {["Animation", "Media Production", "Video Services", "Training Specialist", "Advertising Expert", "Content Creation", "Computer Graphics", "Branding Specialist", "Designs", "Conceptualization", "Mobile and Website Development", "Creative Graphics", "Multimedia Training"].map((skill, i) => (
                        <span key={i} className="px-3 py-1.5 bg-slate-100 text-slate-700 text-sm font-bold rounded-lg border border-slate-200">
                          {skill}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="relative border-l-2 border-indigo-100 ml-3 md:mx-auto md:w-full md:max-w-3xl md:border-l-0">
                    ...
                    {[
                      ...
                      <motion.div 
                        ...
                        className={`relative flex flex-col md:flex-row items-center justify-between mb-12 last:mb-0 ${i % 2 === 0 ? 'md:flex-row-reverse' : ''}`}
                      >
                        {/* Timeline Dot */}
                        <div className="absolute left-[-11px] md:left-1/2 md:-translate-x-1/2 w-6 h-6 rounded-full bg-white border-4 border-[#1B2A6B] shadow-sm z-10 mt-1 md:mt-0"></div>
                        
                        {/* Content Box */}
                        <div className={`w-full pl-8 md:pl-0 md:w-5/12 ${i % 2 === 0 ? 'md:text-left' : 'md:text-right'}`}>
                          <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100 hover:shadow-md hover:border-[#1B2A6B]/20 transition-all">
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
              {[
                ...
                <motion.div
                  ...
                  transition={{ delay: i * 0.1 }}
                  whileHover={{ y: -10 }}
                  className="bg-slate-50 rounded-2xl p-6 border border-slate-100 shadow-sm hover:shadow-xl transition-all group relative overflow-hidden text-center"
                >
                  <div className="w-28 h-28 mx-auto rounded-full overflow-hidden mb-6 bg-slate-200 border-4 border-white shadow-md">
                    <img src={member.img} alt={member.name} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-500" />
                  </div>
                  <h4 className="text-xl font-bold text-slate-900 mb-1">{member.name}</h4>
                  <p className="text-sm font-black text-[#C9A227] uppercase tracking-wider mb-4">{member.role}</p>

                  {/* Socials */}
                  <div className="flex items-center justify-center gap-3 opacity-0 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 transition-all duration-300 mb-2">
                    <a href="#" className="text-slate-400 hover:text-[#1B2A6B] transition-colors"><Linkedin size={18} /></a>
                    <a href="#" className="text-slate-400 hover:text-[#1B2A6B] transition-colors"><Twitter size={18} /></a>
                  </div>
