...
export default function CompanyInternshipsPage() {
  const { data, isLoading, mutate } = useSWR("/company/internships", fetcher);
  
  const [filter, setFilter] = useState("all");
  const [search, setSearch] = useState("");
  const [menuOpen, setMenuOpen] = useState<string | null>(null);

  const companyInternships = data?.data || [];

  const updateStatus = async (id: string, status: string) => {
    try {
      await api.put(`/company/internships/${id}/status`, { status });
      mutate();
  ...
  const filtered = companyInternships.filter((internship: any) => {
    const matchesFilter =
      filter === "all" ||
      (filter === "active" && (internship.status === "active" || internship.status === "draft")) ||
      (filter === "closed" && (internship.status === "closed" || internship.status === "expired"));
    const matchesSearch = internship.title.toLowerCase().includes(search.toLowerCase());
    return matchesFilter && matchesSearch;
  });

  const activeCount = companyInternships.filter((j: any) => j.status === "active").length;
  const draftCount = companyInternships.filter((j: any) => j.status === "draft").length;
  const totalApplicants = companyInternships.reduce((acc: number, j: any) => acc + (j.applicants || 0), 0);
  const totalViews = companyInternships.reduce((acc: number, j: any) => acc + (j.views || 0), 0);

  return (
    <CompanyDashboardLayout>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          <h1 className="text-2xl font-black text-slate-800 mb-1">My Internships</h1>
          <p className="text-slate-500 font-medium text-sm">
            Manage your active internship listings.
          </p>
        </div>
        <Link
        ...
        <div className="divide-y divide-slate-100">
          {filtered.map((internship, i) => (
            ...
            >
              ...
              <div className="col-span-5 flex items-start gap-4">
                ...
                <div>
                  <h3 className="text-sm font-black text-slate-800 mb-1">{internship.title}</h3>
              ...
              <div className="col-span-2 flex justify-center">
                <span className={`px-2.5 py-1 text-[10px] font-black uppercase tracking-wider rounded ${STATUS_COLORS[internship.status] || "bg-slate-100 text-slate-600"}`}>
                  {internship.status}
                </span>
              </div>
              ...
              <div className="col-span-2 flex flex-col items-center">
                <p className="text-sm font-black text-slate-800">{internship.applicants || 0}</p>
                <p className="text-[10px] font-bold text-slate-400">Applicants</p>
                <p className="text-[10px] font-semibold text-slate-400 mt-0.5">{internship.views || 0} views</p>
              </div>
                  ...
                  <>
                    ...
                    <div className="absolute right-0 top-10 w-44 bg-white border border-slate-200 rounded-xl shadow-xl z-20 py-1 overflow-hidden">
                      {internship.status === "active" && (
                        <button
                          onClick={() => { updateStatus(internship.id, "closed"); setMenuOpen(null); }}
                      ...
                      {internship.status === "closed" && (
                        <button
                          onClick={() => { updateStatus(internship.id, "active"); setMenuOpen(null); }}
                          className="flex items-center gap-2 w-full px-4 py-2.5 text-xs font-bold text-slate-600 hover:bg-slate-50"
                        >
                          <CheckCircle size={14} /> Re-open
                        </button>
                      )}
                      <button
                        onClick={() => { toast.success(`Viewing applicants for "${internship.title}"`); setMenuOpen(null); }}
                        className="flex items-center gap-2 w-full px-4 py-2.5 text-xs font-bold text-slate-600 hover:bg-slate-50"
                      >
                      ...
                      <button
                        onClick={() => {
                          if (confirm(`Are you sure you want to completely delete "${internship.title}"?`)) {
          ...
          {!isLoading && filtered.length === 0 && (
            <div className="p-12 text-center">
              <p className="text-slate-500 font-medium mb-4">No internships found.</p>
              <Link
                href="/company/jobs/new"
