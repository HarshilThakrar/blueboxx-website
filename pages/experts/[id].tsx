...
export default function ExpertProfilePage() {
  ...
  const dates = [
    ...
    { day: "Fri", date: 29, available: true },
    { day: "Sat", date: 30, available: false },
    { day: "Sun", date: 31, available: true },
                    ...
                    <div className="pt-6 border-t border-slate-100">
                      <Button 
                        ...
                        onClick={() => {
                          ...
                          // Extract numeric price from "₹999" string for the cart
                          const priceNum = parseInt(service.price.replace(/[^\d]/g, ''), 10) || 999;
                          addToCart({
                            id: `session-${expert.name}-${service.id}-${Date.now()}`,
                            title: `${service.title} with ${expert.name}`,
                            price: priceNum,
                            thumbnail: expert.avatar,
                            type: 'session'
                          });
                      ...
                      >
                        {selectedTime ? `Confirm & Pay ${services[selectedService].price}` : 'Select a time slot'}
                      </Button>
                    </div>
