...
import { ArrowLeft, Save, Image as ImageIcon, Video, FileText, Check, UploadCloud, Link as LinkIcon, DollarSign, Type, AlignLeft, Loader2 } from "lucide-react";
...
export default function AdminEditCoursePage() {
  ...
  const { data: instructors } = InstructorService.useInstructors(1, 100);
  ...
  useEffect(() => {
    if (id) {
      CourseService.getCourse(Number(id)).then(course => {
        setForm({
          ...
          expert_id: course.expert_id?.toString() || "",
          preview_video_url: course.preview_video_url || "",
          demo_pdf_url: course.demo_pdf_url || "",
          landing_page_url: course.landing_page_url || "",
          price: course.price || "",
  ...
  const submitCourse = async (saveAsDraft: boolean) => {
    ...
    try {
      ...
      formData.set('status', saveAsDraft ? 'Draft' : form.status);
      
      await CourseService.update(Number(id), formData);
      toast.success(saveAsDraft ? "Course saved as draft" : "Course updated successfully!");
      router.push("/admin/courses");
    } catch (e) {
      toast.error("Failed to update course. Check fields.");
      setIsDrafting(false);
  ...
  return (
    <AdminDashboardLayout>
      <Head>
        <title>Edit Course | BlueBoxx DA</title>
      </Head>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <h1 className="text-2xl font-black text-[#0d1635]">Edit Course</h1>
            </div>
            <p className="text-slate-500 font-medium text-sm ml-7">Update details and settings for this course.</p>
          </div>
                  ...
                  <div className="flex items-center gap-6">
                    {form.thumbnail ? (
                      <div className="relative group">
                        <img src={getImageUrl(form.thumbnail)} alt="Thumbnail" className="w-48 h-32 object-cover rounded-xl border border-slate-200 shadow-sm" />
                    ...
                    <div className="flex flex-wrap gap-4">
                      {levels?.map(lvl => (
                        <label key={lvl.id} className="flex items-center gap-2 cursor-pointer group">
                          ...
                          <span className="text-sm font-bold text-slate-700 group-hover:text-slate-900">{lvl.name}</span>
                        </label>
                      ))}
                    </div>
                  </div>
