...
export default function AdminAddContestPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newContest = {
      id: Date.now(),
      title: form.title,
      startDate: form.startDate ? new Date(form.startDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : 'TBD',
      endDate: form.endDate ? new Date(form.endDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : 'TBD',
      price: form.price ? `₹${form.price}` : '₹0',
      status: form.status ? form.status.charAt(0).toUpperCase() + form.status.slice(1) : 'Active',
      entries: 0,
    };
    
    const existingData = localStorage.getItem('bb_contests');
    let contests = [];
    if (existingData) {
      contests = JSON.parse(existingData);
    }
    contests.unshift(newContest);
    localStorage.setItem('bb_contests', JSON.stringify(contests));
