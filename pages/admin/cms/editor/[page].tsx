...
export default function AdminPageEditor() {
  ...
  const [pageData, setPageData] = useState({
    title: pageId === 'home' ? 'Homepage' : 'Custom Page',
    hero: {
    ...
    contentHtml: `
      ...
      <p>This is a rich text block where you can add paragraphs, lists, and formatting.</p>
      <ul>
                    ...
                    {pageData.features.map((feature, idx) => (
                      <div key={feature.id} className="p-3 bg-white border border-slate-200 rounded-lg relative group">
                        <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button className="text-rose-500 hover:bg-rose-50 p-1 rounded"><Trash2 size={12} /></button>
                        </div>
                        <p className="text-xs font-bold text-slate-800 mb-1">Item {idx + 1}</p>
                        <input type="text" value={feature.title} className="w-full text-sm font-semibold outline-none border-b border-transparent focus:border-slate-300 mb-1" readOnly />
                        <p className="text-[10px] text-slate-500">{feature.description}</p>
                      </div>
                    ))}
                    <button className="w-full py-2 border-2 border-dashed border-slate-300 rounded-lg text-xs font-bold text-slate-500 hover:bg-slate-100 hover:border-slate-400 transition-colors flex items-center justify-center gap-2">
            ...
            {activeTab === 'settings' && (
              <div className="space-y-4">
                ...
                <div className="space-y-1.5 mt-6">
                  ...
                  <input type="text" defaultValue={`${pageData.title} | BlueBoxx`} className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-sm font-semibold outline-none" />
                </div>
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">SEO Description</label>
                  <textarea rows={3} defaultValue={pageData.hero.subheading} className="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-sm font-semibold outline-none resize-none" />
                </div>
              </div>
            )}
          </div>
