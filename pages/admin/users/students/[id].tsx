...
export default function EditStudent() {
  ...
  const [existingPhotos, setExistingPhotos] = useState<{ profile: string | null; resume: string | null }>({
    profile: null,
    resume: null
  });
  ...
  const fetchStudent = async () => {
    try {
      setIsFetching(true);
      const res = await StudentService.getStudentDetails(id as string);
      const student = res.data;
      const profile = student.student_profile || {};
      const edu = student.education?.[0] || {};
      
      const getSocial = (platform: string) => student.social_links?.find((l: any) => l.platform === platform)?.url || '';
      const getSkillNames = () => student.skills?.map((s: any) => s.skill_name).join(', ') || '';
      ...
      setFormData({
        ...
        github_url: getSocial('github'),
        linkedin_url: getSocial('linkedin'),
        portfolio_url: getSocial('portfolio'),
      });

      setExistingPhotos({
        profile: profile.profile_photo ? `/storage/${profile.profile_photo}` : null,
        resume: student.documents?.find((d: any) => d.type === 'resume')?.file_path 
          ? `/storage/${student.documents.find((d: any) => d.type === 'resume').file_path}` : null,
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await StudentService.updateStudent(id as string, submitData);
    ...
    return (
      <AdminDashboardLayout>
        <div className="flex items-center justify-center min-h-[60vh]">
          <RefreshCw className="animate-spin text-[#C9A227] w-8 h-8" />
        </div>
      </AdminDashboardLayout>
    );
  }
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">Update information for {formData.first_name} {formData.last_name}</p>
            </div>
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit}>
            <div className="p-6 md:p-8 space-y-8">
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-1.5">
                    <label className="text-sm font-bold text-gray-700">Password (Leave blank to keep current)</label>
                    <input type="password" value={formData.password} onChange={(e) => setFormData({...formData, password: e.target.value})} className={`w-full px-4 py-2.5 rounded-lg border border-gray-200 focus:border-[#C9A227] outline-none text-sm`} placeholder="••••••••" />
                  </div>
                  <div className="space-y-1.5">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex-1 relative border-2 border-dashed border-gray-200 rounded-xl px-4 py-4 hover:bg-gray-50 hover:border-[#C9A227]/50 transition-colors cursor-pointer text-center group">
                        ...
                        <p className="text-xs font-medium text-gray-500">{files.profile_photo ? files.profile_photo.name : 'Click to update image'}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex-1 relative border-2 border-dashed border-gray-200 rounded-xl px-4 py-4 hover:bg-gray-50 hover:border-[#1B2A6B]/50 transition-colors cursor-pointer text-center group">
                        ...
                        <p className="text-xs font-medium text-gray-500">{files.resume ? files.resume.name : (existingPhotos.resume ? 'Update current resume' : 'Upload resume')}</p>
                      </div>
                      {existingPhotos.resume && !files.resume && (
                        <a href={existingPhotos.resume} target="_blank" rel="noreferrer" className="text-xs text-blue-600 underline whitespace-nowrap">View Current</a>
                      )}
                    </div>
                  </div>
                </div>
