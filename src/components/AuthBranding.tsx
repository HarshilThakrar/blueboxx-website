        ...
        >
          ...
          <div className="relative overflow-hidden w-full group mask-image:linear-gradient(to_right,transparent,black_10%,black_90%,transparent)">
            <motion.div 
              ...
              className="flex items-center gap-16 whitespace-nowrap opacity-50 grayscale text-white"
            >
              <div className="flex items-center gap-16">
                {partnerCompanies.slice(0, 8).map(company => (
                  <div key={`a-${company.id}`} className="flex items-center justify-center h-8">
                    {company.logoUrl ? (
                      <img src={company.logoUrl} alt={company.name} className="h-full w-auto object-contain brightness-0 invert" />
                    ) : (
                      <span className="text-xl font-black tracking-tighter">{company.name.toUpperCase()}</span>
                    )}
                  </div>
                ))}
              </div>
              ...
              <div className="flex items-center gap-16">
                {partnerCompanies.slice(0, 8).map(company => (
                  <div key={`b-${company.id}`} className="flex items-center justify-center h-8">
                    {company.logoUrl ? (
                      <img src={company.logoUrl} alt={company.name} className="h-full w-auto object-contain brightness-0 invert" />
                    ) : (
                      <span className="text-xl font-black tracking-tighter">{company.name.toUpperCase()}</span>
